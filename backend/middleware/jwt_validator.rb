  ...
  def call(env)
    ...
    if token
      begin
        ...
        logger.info("[#{env['request_id']}] - Invalid Signature in request [#{env['REQUEST_METHOD']} #{env['REQUEST_PATH']}] from #{env['REMOTE_ADDR']}")
        ...
        logger.info("[#{env['request_id']}] - Malformed Token (#{token}) in request [#{env['REQUEST_METHOD']} #{env['REQUEST_PATH']}] from #{env['REMOTE_ADDR']}")
        return Rack::Response.new(
          'Malformed Token',
          401,
